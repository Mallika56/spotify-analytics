...
def clip01(x: float) -> float:
    """Clamp a value into the [0, 1] range audio features are expressed in."""
    return float(np.clip(x, 0.0, 1.0))


def generate_tracks(n: int = N_DEFAULT, seed: int = 7) -> pd.DataFrame:
    """Build a synthetic Spotify-style tracks dataset.

    Audio features are correlated by genre (not independent noise) and
    popularity has a mild, noisy relationship with a few of them, so
    downstream correlation analysis reveals something real. Nulls,
    out-of-range junk, and duplicate rows are injected afterward for
    cleaning practice.
    """
